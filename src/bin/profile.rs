...
fn main() {
    ...
    let grammar = utter::json::parse_string_array(&std::fs::read_to_string(&args[1]).unwrap()).unwrap();
    ...
    for with_iv in [false, true] {
        let mut pipe = FeaturePipeline::new(&opts, if with_iv { model.ivector.as_ref() } else { None });
    ...
    let graph = std::sync::Arc::new(model.compile_grammar(&grammar, None, 5_000_000, |_| {}).unwrap());
    let mut pipe = FeaturePipeline::new(&opts, model.ivector.as_ref());
    let mut nnet = LoopedNnet::new(&model);
    pipe.accept(&samples);
    pipe.input_finished();
    let ready = nnet.num_frames_ready(&pipe);
    let rows: Vec<Vec<f32>> = (0..ready).map(|f| nnet.frame(&mut pipe, f).to_vec()).collect();
    let cfg = utter::decoder::DecoderConfig { beam: model.conf.beam, max_active: model.conf.max_active, min_active: model.conf.min_active, beam_delta: 0.5 };
    ...
    println!("decoder: {:.3} ms per frame ({} frames, max active {}), traceback {:.3} ms", el / rows.len() as f64, rows.len(), max_active, {
        let t = Instant::now();
        for _ in 0..100 {
