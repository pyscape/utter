...
RATE = 16000
COMMANDS = "yes no up down left right on off stop go".split()
DIGITS = "zero one two three four five six seven eight nine".split()
FILLERS = "backward forward follow learn visual bed bird cat dog happy house marvin sheila tree wow".split()
LETTERS = list("abcdefghijklmnopqrstuvwxyz")
NATO = ("alpha bravo charlie delta echo foxtrot golf hotel india juliet kilo lima mike november oscar "
        "papa quebec romeo sierra tango uniform victor whiskey yankee zulu").split() + ["x ray"]
COLOURS = "red yellow blue black white".split()
...
def main():
    ...
    args = ap.parse_args()
    data = Path(args.data)
    listed = [l.strip() for l in (data / f"{args.split}_list.txt").read_text().splitlines() if l.strip()]
    ...
    report = {"split": args.split, "clips": len(clips), "block_ms": args.block_ms, "grammar_size": len(full_grammar), "engines": {}}
    lines = [f"# Speech Commands v2, {args.split} split, {len(clips)} clips, {args.block_ms} ms blocks", ""]
    lines.append(f"Grammar: the dataset's 35 words plus {len(LETTERS)} letters, {len(NATO)} NATO words and {len(COLOURS)} colours, {len(full_grammar)} entries.")
    lines.append("")
    ...
    for name in ["vosk"] + [n for n in modules if n != "vosk"]:
        ...
        results[name] = dict(correct=correct, total=len(clips), confusions=conf.most_common(15), per_word={w: (per_word_ok[w], per_word[w]) for w in DATASET_WORDS},
                             latency_ms_p50=quantile(lat, 0.5), latency_ms_p90=quantile(lat, 0.9), latency_n=len(lat), never_in_partial=never, rtf=wall / audio if audio else None)
    lines.append("## Full grammar: accuracy")
    lines.append("")
    lines.append("| engine | correct | accuracy | never in a partial | first appearance after vosk's word end p50 / p90 | RTF |")
    lines.append("|---|---|---|---|---|---|")
    for name, r in results.items():
        lines.append(f"| {name} | {r['correct']} / {r['total']} | {pct(r['correct'], r['total']):.2f}% | {r['never_in_partial']} | {r['latency_ms_p50']:.0f} / {r['latency_ms_p90']:.0f} ms over {r['latency_n']} | {r['rtf']:.4f} |")
    lines.append("")
    lines.append("| word | " + " | ".join(results) + " |")
    lines.append("|---|" + "---|" * len(results))
    for w in DATASET_WORDS:
        lines.append(f"| {w} | " + " | ".join(f"{r['per_word'][w][0]} / {r['per_word'][w][1]}" for r in results.values()) + " |")
    lines.append("")
    for name, r in results.items():
        lines.append(f"Most frequent confusions, {name}: " + "; ".join(f"{a} -> {b} ({n})" for (a, b), n in r["confusions"][:10]))
    lines.append("")
    ...
    lines.append("")
    lines.append("| engine | commands correct | fillers and digits read as unknown | fillers and digits read as a command | noise seconds | noise seconds with a word |")
    lines.append("|---|---|---|---|---|---|")
    twelve = {}
    for name, mod in modules.items():
        ...
        if name != "vosk" and args.unknown_cost:
            eng.new = lambda eng=eng: (lambda r: (r.SetWords(True), r)[1])(eng.mod.KaldiRecognizer(eng.model, RATE, json.dumps(COMMANDS), args.unknown_cost))
        ...
        twelve[name] = dict(cmd_ok=cmd_ok, cmd_total=cmd_total, other_unk=other_unk, other_cmd=other_cmd, other_total=other_total, noise_seconds=noise_seconds, noise_words=noise_words)
        lines.append(f"| {name} | {cmd_ok} / {cmd_total} ({pct(cmd_ok, cmd_total):.2f}%) | {other_unk} / {other_total} ({pct(other_unk, other_total):.2f}%) | {other_cmd} ({pct(other_cmd, other_total):.2f}%) | {noise_seconds} | {noise_words} |")
